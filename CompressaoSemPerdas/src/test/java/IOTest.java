...
public class IOTest {
    ...
    public void writingDictionaryToAndReadingFromFilesWorks() {
        List<LZ77DictionaryEntry> originaDictionary = compressor.compress(stringToHandle, 127, 7);
        io.writeDictionaryToFile("test2", originaDictionary);
        List<LZ77DictionaryEntry> dictionaryFromFile = io.compressedDataFromFile("test2");
        String decompressedString = decompressor.decompress(dictionaryFromFile);

        assertEquals(stringToHandle, decompressedString);
    }
}
