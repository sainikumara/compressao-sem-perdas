...
public class LZ77 {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<LZ77DictionaryEntry> dictionary = new LZ77Compress().compress(stringToCompress, searchWindowLength, lookAheadWindowLength);
        new CompressionInfo().LZ77Info(stringToCompress, dictionary, searchWindowLength, lookAheadWindowLength);

        io.writeDictionaryToFile("/resources/compressed", dictionary);
        List<LZ77DictionaryEntry> reconstructedDictionary = io.compressedDataFromFile("/resources/compressed");

        String decompressedString = new LZ77Decompress().decompress(reconstructedDictionary);
